...
local minimapButton = CreateFrame("Button", "GhostReconMinimapButton", Minimap)
minimapButton:SetSize(32, 32)
minimapButton:SetFrameStrata("MEDIUM")
minimapButton:SetPoint("TOPLEFT", Minimap, "TOPLEFT", 10, -10)
minimapButton:SetNormalTexture("Interface\\AddOns\\GhostRecon\\Recon.tga")
minimapButton:SetHighlightTexture("Interface\\Minimap\\UI-Minimap-ZoomButton-Highlight")
minimapButton:SetScript("OnClick", function(self, button)
  if button == "RightButton" then
    Settings.OpenToCategory(GhostRecon.optionsFrame.settingsCategory:GetID())
  else
...
function GhostRecon:WhereAmI()
  ...
  if isIn then
    ...
    if difficultyID == 2 then
      name = "Heroic " .. name --英雄5人本
    elseif difficultyID == 3 then
      name = name .. "(10)" --10 Player普通10人团本
    elseif difficultyID == 4 then
      name = name .. "(25)" --25 Player普通25人团本
    elseif difficultyID == 5 then
      name = name .. "(10H)" --10 Player (Heroic)英雄10人团本
    elseif difficultyID == 6 then
      name = name .. "(25H)" --25 Player (Heroic)英雄25人团本
    elseif difficultyID == 7 then
      name = name .. "(LFRlegacy)" --随机团本Legacy LFRs prior to SoO
    elseif difficultyID == 8 then
      name = name .. "(Challenge)" --Mythic Keystone史诗钥石
    elseif difficultyID == 9 then
      name = name .. "(40)" --40 Player 40人团本
    elseif difficultyID == 11 then
      name = name .. "(HSC)" --Heroic Scenario英雄场景战役
    elseif difficultyID == 12 then
      name = name .. "(SC)" --Normal Scenario普通场景战役
    elseif difficultyID == 14 then
      name = name .. "(Raid)" --Normal Raid普通团本
    elseif difficultyID == 15 then
      name = name .. "(HRaid)" --Heroic Raid英雄团本
    elseif difficultyID == 16 then
      name = name .. "(MRaid)" --Mythic Raid史诗团本
    elseif difficultyID == 17 then
      name = name .. "(LFR)" --Looking For Raid随机团本
    elseif difficultyID == 23 then
      name = name .. "(M5P)" --Mythic party史诗5人本
    elseif difficultyID == 24 then
      name = name .. "(TW)" --Timewalking party时光5人本
    elseif difficultyID == 33 then
      name = name .. "(TW)" --Timewalking raid时光团本
    end
  end
  return name
end
...
local function OnEvent(this, event, timeStamp, ...)
  if event == "COMBAT_LOG_EVENT_UNFILTERED" then
    local timeStamp, combatEvent, hideCaster, sourceGuid, sourceName, sourceFlags, sourceRaidFlags, destGuid, destName, destFlags, destRaidFlags, spellId, spellName, spellSchool, missType =
        ...
        if combatEvent == "SPELL_DISPEL" then
          if GhostReconDB.Settings.Active and (inInst or not GhostReconDB.Settings.InstancesOnly) then
            ...
            if UnitIsFriend(destName, "player") then
              ...
              local spellInfo = C_Spell.GetSpellInfo(dispelledSpellId)
              local extraName = spellInfo and spellInfo.name
  ...
  if event == "ADDON_LOADED" then
    ...
    if whichAddon == "GhostRecon" then
      ...
      GhostRecon.useDefaultSlash = not chatEnabled
