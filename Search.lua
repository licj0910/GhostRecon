...
local function ReportCurrentMob(who)
  if frm.spellFrame.spellCount > 0 then
    ...
    GhostRecon:Announce(string.format(L["Ghost: Recon - Spells and Abilities of '%s'..."], name), who)
...
local function CreateSpellFrame(spellId, parent)
  ...
  rc.SetSpell = function(this, spellId)
    ...
    local image = spellInfo and spellInfo.iconID

    this.spellImage:SetTexture(image)
    this.spellId = spellId
    this.hyperlink = C_Spell.GetSpellLink(spellId)
...
frm.SpellLabel:SetText(L["Abilities"])
...
frm.Title:SetText(L["Ghost: Recon - Mob Search"])
...
frm.NotesLabel:SetText(L["Notes"])
...
frm.ClearMob:EnableMouse(true)
frm.ClearMob:SetText(L["Clear Mob"])
frm.ClearMob:SetWidth(112)
frm.ClearMob:SetHeight(24)
frm.ClearMob:Disable()
-- confirmation dialog
StaticPopupDialogs["GhostReconClearMob"] = {
  text = L["Do you really want to remove information about this mob from the database?"],
  button1 = "Yes",
  button2 = "No",
  whileDead = 1,
  hideOnEscape = 1,
...
frm.ReportMob:EnableMouse(true)
frm.ReportMob:SetText(L["Report"])
